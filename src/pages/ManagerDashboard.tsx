...
import { mockApi, Submission } from '@/lib/mockApi';
...
const ManagerDashboard = () => {
  ...
  const { data: submissions, isLoading } = useQuery({
    ...
    queryFn: mockApi.submission.getAll,
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'approved' | 'rejected' }) =>
      mockApi.submission.updateStatus(id, status),
    onSuccess: () => {
  ...
  const statCards = [
    ...
    { 
      ...
      value: `${((stats?.totalEmissions ?? 0) / 1000).toFixed(1)}t`, 
      icon: Factory, 
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge variant="outline" className="bg-status-pending-bg text-status-pending border-status-pending/30">Pending</Badge>;
      case 'approved':
        return <Badge variant="outline" className="bg-status-approved-bg text-status-approved border-status-approved/30">Approved</Badge>;
      case 'rejected':
        return <Badge variant="outline" className="bg-status-rejected-bg text-status-rejected border-status-rejected/30">Rejected</Badge>;
      default:
                ...
                <TableBody>
                  {submissions?.map((submission, index) => (
                    ...
                    >
                      ...
                      <TableCell>{submission.supplierName}</TableCell>
                      <TableCell className="text-muted-foreground">{submission.supplierCompany}</TableCell>
                      <TableCell className="text-right font-mono">
                        {submission.totalEmissions.toLocaleString()} kg
                      </TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Review the extracted data and approve or reject this submission
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="aspect-[3/4] bg-muted rounded-lg flex items-center justify-center border border-border">
                  <div className="text-center text-muted-foreground p-4">
                    ...
                    <p className="text-sm">invoice_{selectedSubmission.id}.pdf</p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between p-3 bg-muted/50 rounded-lg">
                    ...
                    <span className="font-mono font-medium">{selectedSubmission.electricity} kWh</span>
                  </div>
                  <div className="flex justify-between p-3 bg-muted/50 rounded-lg">
                    <span className="text-muted-foreground">Natural Gas</span>
                    <span className="font-mono font-medium">{selectedSubmission.gas} m³</span>
                  </div>
                  <div className="flex justify-between p-3 bg-muted/50 rounded-lg">
                    <span className="text-muted-foreground">Fuel</span>
                    <span className="font-mono font-medium">{selectedSubmission.fuel} L</span>
                  </div>
                  <div className="flex justify-between p-3 bg-muted/50 rounded-lg">
                    <span className="text-muted-foreground">Waste</span>
                    <span className="font-mono font-medium">{selectedSubmission.waste} kg</span>
                  </div>
                  <div className="flex justify-between p-3 bg-accent rounded-lg">
                    ...
                    <span className="font-mono font-bold text-primary">
                      {selectedSubmission.totalEmissions.toLocaleString()} kg
                    </span>
                  </div>
                </div>

                {/* Audit Hash Section */}
                {selectedSubmission.status === 'approved' && selectedSubmission.auditHash && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mt-4 p-4 bg-status-approved-bg rounded-lg border border-status-approved/20"
                  >
