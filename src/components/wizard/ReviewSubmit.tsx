...
const ReviewSubmit = () => {
  ...
  // Merge data: AI extracted takes priority over manual
  const mergedData = aiExtractedData || manualData;
  
  // Calculate CO2e (simplified formula)
  const calculateCO2e = () => {
    return Math.round(
      mergedData.electricity * 0.5 +
      mergedData.gas * 2.0 +
      mergedData.fuel * 2.5 +
      mergedData.waste * 0.3
    );
  };

  const fields = [
    { key: 'electricity', label: 'Electricity', unit: 'kWh', icon: Zap, color: 'text-amber-500', bgColor: 'bg-amber-50' },
    { key: 'gas', label: 'Natural Gas', unit: 'm³', icon: Flame, color: 'text-orange-500', bgColor: 'bg-orange-50' },
    { key: 'fuel', label: 'Fuel', unit: 'L', icon: Droplets, color: 'text-blue-500', bgColor: 'bg-blue-50' },
    { key: 'waste', label: 'Waste', unit: 'kg', icon: Trash2, color: 'text-slate-500', bgColor: 'bg-slate-50' },
  ];

  const handleSubmit = async () => {
    setIsSubmitting(true);
    try {
      await mockApi.submission.submit(mergedData);
      setSubmissionComplete(true);
      toast.success('Submission successful! Your data has been recorded.');
    ...
    >
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Verify the extracted data below before submitting
        </p>
      </div>

      {/* Data Fields */}
      <div className="grid gap-4 sm:grid-cols-2">
        {fields.map(({ key, label, unit, icon: Icon, color, bgColor }, index) => {
            ...
            >
              ...
              {isAIFilled && (
                <motion.div
                  ...
                  className="absolute -top-2 -right-2 z-10 flex items-center gap-1 px-2 py-1 rounded-full bg-primary text-primary-foreground text-xs font-medium shadow-lg"
                >
                ...
                <CardContent className="pt-4">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      value={value}
                      readOnly
                      className="h-12 pr-16 text-lg font-semibold bg-muted/50"
                    />
        ...
        <Card className="bg-gradient-to-br from-primary/5 via-primary/10 to-accent border-primary/20">
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Based on standard emission factors for Scope 1 & 2 emissions
            </p>
          </CardContent>
        </Card>
      </motion.div>
